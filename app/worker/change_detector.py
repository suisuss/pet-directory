...
class ChangeDetector:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(f"Connected to database for change detection: {db_url}")
    ...
    def process_notification(self, connection, pid: int, channel: str, payload: str) -> None:
        ...
        try:
            ...
            operation_key = f"{event.operation.lower()}s"
            if operation_key in self.stats:
                self.stats[operation_key] += 1
    ...
    async def listen(self) -> None:
        ...
        try:
            ...
            await self.connection.remove_listener("pet_changes", self.process_notification)
            await self.disconnect()
    
    async def log_statistics(self) -> None:
        """Log current statistics."""
        uptime = datetime.now() - self.stats["started_at"] if self.stats["started_at"] else "N/A"
        logger.info(
